...
│                 + Admin（栏目/文章 CRUD 编排）               │
├─────────────────────────────────────────────────────────────┤
│ L4 数据访问层   Django ORM（QuerySet、迁移、认证）            │
│                 ↓                                           │
...
| --- | --- | --- |
| ADR-1 | 技术栈定稿：Python 3.12 + Django 5.2 LTS + SQLite + Bootstrap 5 | 本机实测 Python 3.12.9；LTS 保证稳定性；SQLite 零部署成本 |
...
│   ├── M1.3 只读查询子模块    selectors.py
│   ├── M1.4 分页工具子模块    pagination.py
│   ├── M1.5 视图控制子模块   views.py + urls.py
...
| M1.3 selectors | 前台只读查询、日期边界、N+1 优化 | M1.1 | ≈80 行 |
| M1.4 pagination | 统一页大小、页码守卫、查询参数保留 | Django Paginator | ≈35 行 |
| M1.5 views | 4 个视图函数：HTTP 编排、上下文和异常分流 | M1.1~M1.4 | ≈100 行 |
...
| `ItemQuerySet.published()` | 统一公开文章过滤链 | M1.1 → L4 |
| `selectors.search_published_items()` | 组合关键词/日期/栏目查询 | M1.3 → M1.1 → L4 |
| `pagination.paginate()` | 安全分页对象 | M1.5 → M1.4 |
...
| Item | id, title, content, publish_time(索引), updated_at, is_published, author | N:1 引用 Category（PROTECT） | SQLite 表 `content_item` |
| User | 内置 auth 模型，is_staff 判定管理角色 | 1:N 作者关系（SET_NULL） | auth_user 内置表 |

**设计要点（概要）**：
- 关系完整性由数据库外键约束保证（PROTECT 策略——有文章的栏目禁删）；
...
浏览器 GET /search/?q=Python&category=2
  → M0 路由 → M1.3 search()
  → M1.2 校验（失败：错误回显，不落库）
→ M1.3 selectors 逐条件 filter（AND） → L4 参数化 SQL
  → 分页 → M2 模板渲染（表单回显） → 200
```
...
| 控制点 | 手段 |
| --- | --- |
| 数据访问 | ORM 参数化（禁拼接 SQL）、QuerySet 链式过滤 |
