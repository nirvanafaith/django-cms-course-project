...
graph TB
    ...
    subgraph 数据访问层
    ORM[Django ORM / QuerySet / ModelChoiceField]
...
→ 校验通过 → selectors 逐条件 filter（AND）构造 QuerySet
...
│   │   ├── test_querysets.py    # published() 可见性规则
│   │   ├── test_selectors.py    # 只读查询与 N+1 约束
...
| 模块/文件 | 职责 | 依赖 |
| --- | --- | --- |
| content/models.py | 定义实体、数据约束与 published() 查询集合 | django.db |
| content/selectors.py | 集中前台只读查询、日期边界和 N+1 优化 | models+django ORM |
| content/pagination.py | 统一页大小、页码守卫和查询参数保留 | django.core.paginator |
| content/forms.py | SearchForm：GET 查询参数校验（日期格式/顺序/长度） | django.forms |
| content/views.py | HTTP 请求读取、模块调用、上下文组装和响应返回 | forms+selectors+pagination |
...
| category | ModelChoiceField(required=False) | 选项来自 Category QuerySet；非法 id → 校验错误 | Category 或 None |
...
**search(request)** —— HTTP 编排入口（三种模式组合逻辑由 selector 承担）
...
**item_detail**：调用 `published_item_by_pk(pk)`，草稿和不存在文章均转换为 HTTP 404 → 模板渲染。

**index**：调用 `categories_with_item_counts()` 和 `latest_published_items()`；栏目只统计已发布文章，首页文章使用 `select_related("category")`。
...
| 权限 | 仅 `is_staff` 可登录后台；超级用户：`createsuperuser` 创建默认账号（部署说明记录 `ctx/1234321` 属示例，生产建议强密码） |
